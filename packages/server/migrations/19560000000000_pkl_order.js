/**
 * PKL line order lifecycle — own table keyed by mother coil_no (N slits per order).
 * @param {import('node-pg-migrate').MigrationBuilder} pgm
 */
exports.up = (pgm) => {
  pgm.sql(`
    CREATE TABLE IF NOT EXISTS txn.pkl_order (
      order_id                BIGSERIAL    PRIMARY KEY,
      coil_no                 VARCHAR(30)  NOT NULL UNIQUE REFERENCES coil.coil(coil_no),
      mother_coil_no          VARCHAR(30),
      slit_id                 VARCHAR(20),
      ...
      machine_code            VARCHAR(8)   NOT NULL DEFAULT 'PKL'
                              REFERENCES master.machine(machine_code)
                              CHECK (machine_code = 'PKL'),
    ...
    CREATE INDEX IF NOT EXISTS ix_pkl_order_status
      ON txn.pkl_order (status);
    CREATE INDEX IF NOT EXISTS ix_pkl_order_shift_status
      ON txn.pkl_order (shift_log_id, status);

    ALTER TABLE txn.stoppage
      ADD COLUMN IF NOT EXISTS pkl_order_id BIGINT REFERENCES txn.pkl_order(order_id) ON DELETE CASCADE;

    ALTER TABLE txn.stoppage DROP CONSTRAINT IF EXISTS stoppage_order_kind_check;
    ALTER TABLE txn.stoppage
      ADD CONSTRAINT stoppage_order_kind_check
      CHECK (order_kind IN ('CRM','RWD','HRS','PKL'));

    CREATE INDEX IF NOT EXISTS ix_stoppage_pkl_open
      ON txn.stoppage (pkl_order_id) WHERE pkl_order_id IS NOT NULL AND end_at IS NULL;
  `);
};
...
exports.down = (pgm) => {
  pgm.sql(`
    DROP INDEX IF EXISTS txn.ix_stoppage_pkl_open;
    ALTER TABLE txn.stoppage DROP COLUMN IF EXISTS pkl_order_id;
    ALTER TABLE txn.stoppage DROP CONSTRAINT IF EXISTS stoppage_order_kind_check;
    ALTER TABLE txn.stoppage
      ADD CONSTRAINT stoppage_order_kind_check
      CHECK (order_kind IN ('CRM','RWD','HRS'));
    DROP TABLE IF EXISTS txn.pkl_order;
  `);
};
