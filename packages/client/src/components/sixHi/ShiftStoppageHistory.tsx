            ...
            <tbody className="divide-y divide-border">
              {stoppages.map((s) => (
                <tr key={s.id} className="hover:bg-muted/5 transition-colors">
                  <td className="px-4 py-2 font-mono text-foreground">{new Date(s.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</td>
                  <td className="px-4 py-2 font-mono text-muted-foreground">{s.endTime ? new Date(s.endTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Ongoing'}</td>
                  <td className="px-4 py-2 font-mono text-warning font-medium">{s.durationMins != null ? `${s.durationMins}m` : '—'}</td>
