...
export type HrsEntryShape = {
  ...
  hrsCapture?: Record<string, unknown> | null;
};

export type HrsOrderShape = {
  orderId?: string;
  coilNo?: string;
  status?: string;
  gradeCode?: string;
...
/** HRS MH order side panel — summary + drawer CTAs. */
export function HrsSidePanel({
  card,
  detail,
  detailLoading,
  busy,
  ...
  detail: HrsCoilDetail | null;
  detailLoading: boolean;
  busy: boolean;
  onShowHrsDetails: () => void;
  ...
  return (
    <div className="bg-white border border-border rounded-2xl flex flex-col shadow-sm overflow-hidden max-h-[min(720px,calc(100vh-12rem))]">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 py-3">
        ...
        <dl className="grid grid-cols-2 gap-x-3 gap-y-2.5 text-sm">
          ...
          <div>
            ...
            <dd className="font-semibold mt-0.5">HR Slitting</dd>
          </div>
          {holdReason && (
      ...
      <div className="shrink-0 p-4 border-t border-border space-y-2">
        <ZButton variant="primary" fullWidth onClick={onShowHrsDetails} className="gap-2 min-h-12">
