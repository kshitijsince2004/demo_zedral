 ...
 */
router.get('/', requireRole([UserRole.PLANT_HEAD, UserRole.MACHINE_HEAD, UserRole.SUPERVISOR, UserRole.ADMIN]), async (req, res) => {
  try {
 ...
 */
router.get('/suggest', requireRole([UserRole.PLANT_HEAD, UserRole.MACHINE_HEAD, UserRole.SUPERVISOR, UserRole.ADMIN]), async (req, res) => {
  try {
    const query = req.query.q as string;
