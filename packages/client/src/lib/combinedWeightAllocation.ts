...
/**
 * Resolve the combined actual weight shown in the form.
 ...
 * then larger units — mirrors legacy combined-run fill behaviour.
 */
export function allocateCombinedWeight(
  ...
  const sorted = [...targets].sort((a, b) => a.targetMt - b.targetMt || a.batchNumber.localeCompare(b.batchNumber));
