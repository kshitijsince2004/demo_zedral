...
import type { AnnBoardRow } from './bodies/AnnBaseCard';
import { AnnBaseAssignModal } from './bodies/AnnBaseAssignModal';
import { AnnQueueOrderDetailDrawer } from './AnnQueueOrderDetailDrawer';
...
/** Charge builder — stack bottom→top (seq 1 = bottom). Shared by MH + operator. */
export function AnnBatchingWorkspace() {
  ...
  function addToStack(coilNo: string) {
    ...
    setBatchNo((prev) => {
      ...
      const card = queue.find((q) => q.coilNo === coilNo);
      return (card?.annealingBatch ?? '').trim() || prev;
    });
  }
  ...
  async function createBatch() {
    ...
    try {
      ...
      const planBatch = stack
        .map((coilNo) => queue.find((q) => q.coilNo === coilNo)?.annealingBatch?.trim())
        .find((v) => Boolean(v));
      const bn = batchNo.trim() || planBatch || undefined;
      ...
      setBatchNo('');
      setBaseNo('');
      setOkMsg(`Created ${cn}${bn ? ` · batch ${bn}` : ''}`);
      await reload();
    } catch (e: unknown) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <div className="grid min-h-0 flex-1 items-stretch gap-3 overflow-hidden p-3 lg:grid-cols-[1fr_1fr_14rem]">
            ...
            {available.map((c) => (
              <li key={c.coilNo} className="relative">
                ...
                >
                  ...
                  <span className="mt-0.5 block text-[11px] font-mono tabular-nums text-muted-foreground">
                    W {c.widthMm != null && Number(c.widthMm) > 0 ? Number(c.widthMm) : '—'}
                    {' · '}T {c.thicknessMm != null && Number(c.thicknessMm) > 0 ? Number(c.thicknessMm) : '—'}
                  </span>
                </button>
                <button
                  type="button"
                  className="absolute right-2 top-2 inline-flex h-8 w-8 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        ...
        <section className="flex min-h-0 flex-col overflow-hidden max-h-full rounded-lg border border-border bg-background shadow-sm">
          ...
          <div className="flex min-h-0 flex-1 flex-col-reverse overflow-hidden p-2 gap-1">
            {stack.length === 0 && (
        ...
        <aside className="flex min-h-0 flex-col overflow-hidden max-h-full rounded-lg border border-border bg-background shadow-sm">
          ...
          <ul className="min-h-0 flex-1 overflow-hidden p-2 space-y-1 text-xs">
            {unassigned.length > 0 && (
              <>
