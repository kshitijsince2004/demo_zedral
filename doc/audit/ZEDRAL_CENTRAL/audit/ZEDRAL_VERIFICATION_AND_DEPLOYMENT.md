...
- [ ] **Fix the CRM guard** (A.1 ⚠️1) → use `assertMachineAccess` (or map mill→`ROLLING`). **Go-live blocker.**
- [ ] **Finish client ST login** (A.3 ⚠️2) → rework `Login.tsx`/`authStore` to use ST cookies (stop reading `accessToken` from the body), add a staff email+password screen. **Go-live blocker.**
- [ ] **Add SuperTokens to `deploy/docker-compose.prod.yml`** + ST env (Part B blocker).
- [ ] Update `seed-crm6-ppc.mjs` process code `6HI`→`ROLLING` (minor).
- [ ] Run `npm run build && npm test && npm run arch:check` — must be green.
...
- [ ] Set the GitHub repo secrets (Part D) and a `production` environment with required reviewers.
