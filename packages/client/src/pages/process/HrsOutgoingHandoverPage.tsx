...
/** Process-station outgoing handover — HRS slit metrics or RWD queue (not CRM mill form). */
export function HrsOutgoingHandoverPage({ machineCode = 'HRS' }: { machineCode?: string } = {}) {
  const MACHINE_CODE = machineCode;
  ...
  if (loading) {
    return (
      <div className="flex h-full min-h-0 flex-1 items-center justify-center">
        <p className="text-sm text-muted-foreground">Loading HRS handover…</p>
      </div>
    );
  }
    ...
    <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="shrink-0 px-5 py-4 border-b border-border bg-card flex items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-lg font-bold">HRS Slitting · Shift {p.shift.shiftCode}</h1>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-6">
        <section className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <LockedField label="Target MT" value={metrics ? metrics.targetMt.toFixed(2) : '—'} />
            <LockedField label="Production MT" value={metrics ? metrics.totalProdMt.toFixed(2) : '—'} />
            <LockedField label="Scrap MT" value={metrics ? metrics.scrapMt.toFixed(2) : '—'} />
            <LockedField label="Scrap %" value={metrics ? `${metrics.scrapPct}%` : '—'} />
            <LockedField label="Coils" value={metrics?.coilsDone} />
            <LockedField label="Settings" value={metrics?.settingCount} />
          </div>
        </section>

        {/* ponytail: stoppage table when hrs/shift-review exists */}
        <section className="rounded-xl border border-border bg-card p-5">
